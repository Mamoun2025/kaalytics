/* ============================================
   ...
   Beautiful hand-crafted icons by Tailwind
   https://heroicons.com - MIT License
   ============================================ */

const HeroIcons = (function() {
    'use strict';

    // === ICON DEFINITIONS (24x24 outline) ===
    const icons = {
        // Dimensions
    ...
    const dimensionIcons = {
        // Secteurs d'activité (en premier)
        'construction': 'building-office',
        'mining': 'cube',
        'manufacturing': 'cog-6-tooth',
        'logistics': 'truck',
        // Fonctions transversales
        'operations': 'wrench-screwdriver',
        'growth': 'currency-dollar',
        'engagement': 'megaphone',
        'intelligence': 'presentation-chart-line',
        'brand': 'paint-brush',
        'integration': 'link',
        'enterprise': 'building-office',
        'security': 'shield-check'
    };

    // === CREATE SVG ELEMENT ===
    function createSvg(iconName, options = {}) {
        const {
            size = 24,
            strokeWidth = 1.5,
            className = '',
        ...
        if (!path) {
            console.warn(`[HeroIcons] Icon "${iconName}" not found`);
            return null;
        }
    ...
    // === GET SVG STRING ===
    function getSvgString(iconName, options = {}) {
        const {
            size = 24,
            strokeWidth = 1.5,
            className = '',
        ...
        return `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="${strokeWidth}" stroke="${color}" width="${size}" height="${size}" class="${className}">${path}</svg>`;
    }

    // === GET DIMENSION ICON ===
    function getDimensionIcon(dimensionId, options = {}) {
        const iconName = dimensionIcons[dimensionId] || 'cube';
        return getSvgString(iconName, options);
    }

    // === GET DIMENSION ICON ELEMENT ===
    function getDimensionIconElement(dimensionId, options = {}) {
        const iconName = dimensionIcons[dimensionId] || 'cube';
        return createSvg(iconName, options);
    }
